...
- 2026-08-03 C3: serialize the PM2 control plane per Worker and keep one PM2 connection for the manager lifetime; Windows PM2 can throw `client.sock` during immediate reconnect/disconnect even when individual calls succeed. Close the connection from Runtime composition shutdown, and keep the regression test on the shared manager boundary.
- 2026-08-03 C6: the frozen Runtime MCP read qualification requires `server/discover`, `tools/list`, and `tools/call`; `initialize` is not a readiness gate. A real resource reported `reachable=false` still blocks device qualification.
