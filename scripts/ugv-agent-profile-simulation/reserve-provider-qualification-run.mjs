...
const outputName = `smpp-provider-qualification-${runId}.redacted.json`;
const markerName = `smpp-provider-qualification-${runId}.used.json`;
const outputPath = resolve(attemptsDir, outputName);
const markerPath = resolve(attemptsDir, markerName);
if (existsSync(outputPath)) fail("UAP_PROVIDER_QUALIFICATION_RUN_ID_ALREADY_USED");
...
try {
  ...
  writeFileSync(
    ...
    `${JSON.stringify(
      {
        schemaVersion: "ugv-agent-profile.provider-qualification-run-reservation/v1",
        runId,
        generatedAt,
  ...
  if (error?.code === "EEXIST") fail("UAP_PROVIDER_QUALIFICATION_RUN_ID_ALREADY_USED");
  fail("UAP_PROVIDER_QUALIFICATION_RUN_ID_RESERVATION_FAILED");
} finally {
  if (descriptor !== undefined) closeSync(descriptor);
}

process.stdout.write(`UAP_PROVIDER_QUALIFICATION_RUN_ID_RESERVED: ${runId}\n`);
