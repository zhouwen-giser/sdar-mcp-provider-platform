...
try {
  ...
  composeExec("pms-web", [
    "node",
    "-e",
    String.raw`Promise.all(["/","/assets/main.js","/styles.css","/providers/one","/health/live","/health/ready"].map(async p=>{const r=await fetch("http://127.0.0.1:8080"+p);if(!r.ok)throw Error(p);const t=await r.text();if(p==="/"&&(!t.includes("http://pms-api:8090")||r.headers.get("x-content-type-options")!=="nosniff"))throw Error("web")})).then(()=>fetch("http://pms-api:8090/health/ready")).then(r=>{if(!r.ok)process.exit(1)})`,
  ]);
