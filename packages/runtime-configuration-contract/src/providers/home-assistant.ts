...
const HomeAssistantClimateInputBaseSchema = z.object({
  ...
  ADAPTER_PORT: z.coerce.number().int().min(1).max(65_535).default(7020),
...
export const HomeAssistantClimateResolvedSchema = HomeAssistantClimateInputBaseSchema.extend({
...
export type HomeAssistantClimateConfiguration = z.infer<
  typeof HomeAssistantClimateResolvedSchema
> & {
  homeAssistantToken: string;
};

export function loadHomeAssistantClimateConfiguration(
  environment: NodeJS.ProcessEnv,
): HomeAssistantClimateConfiguration {
...
export function homeAssistantClimateLogContext(
  configuration: HomeAssistantClimateConfiguration,
...
const configurationKeys = Object.keys(HomeAssistantClimateResolvedSchema.shape);
const configurationSchema = z.toJSONSchema(HomeAssistantClimateResolvedSchema);
...
export const HomeAssistantClimateConfigurationDefinition = parseConfigurationDefinition({
  schemaVersion: "1.0",
  definitionId: "provider.homeAssistantClimate",
  definitionVersion: 1,
  configGroup: "provider.homeAssistantClimate",
  ...
  defaults: {
    ...
    PROVIDER_TELEMETRY_ENDPOINT: "127.0.0.1:7002",
  ...
  fields: configurationKeys.map((key) => {
    ...
    return {
      path: `/${key}`,
      displayName: key,
      description: `Home Assistant Climate Provider ${key.toLowerCase()} setting.`,
