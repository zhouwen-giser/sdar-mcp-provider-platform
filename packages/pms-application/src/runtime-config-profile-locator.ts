...
export function parseRuntimeConfigProfileLocator(value: string): RuntimeConfigProfileLocator {
  ...
  if (
    parts.length !== 7 ||
    parts[0] !== PROFILE_PREFIX ||
    parts[1] !== PROFILE_VERSION
  ) {
  ...
  if (!RUNTIME_PROFILE_TYPES.includes(targetType)) {
    throw new TypeError("Runtime config profile target type must be runtime_deployment or runtime_instance");
  }
...
export function formatRuntimeConfigProfileLocator(
  locator: RuntimeConfigProfileLocator,
): string {
  const targetType = validateTargetType(locator.targetType);
  const environment = String(locator.environment);
  const targetId = String(locator.targetId);
  const configGroup = String(locator.configGroup);
  const dataId = String(locator.dataId);
  if (
...
export function toConfigurationTarget(
  locator: RuntimeConfigProfileLocator,
): ConfigurationTarget {
  return Object.freeze({
...
function validateTargetType(value: string): ConfigurationTargetType {
  ...
  return value as ConfigurationTargetType;
}
