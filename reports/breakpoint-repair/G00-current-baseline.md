...
| ----------- | --------------------------- | ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| BP-SMPP-001 | `STILL_REPRODUCIBLE`        | `PARTIALLY_FIXED`       | Main completed a Climate Task after the first matching observation. It did not require a stable duration or multiple observations, so a transient `off -> cool -> off` sequence could be committed as success. The repair is implemented in the working tree and awaits final-candidate verification.                                                      |
| BP-SMPP-002 | `STILL_REPRODUCIBLE`        | `PARTIALLY_FIXED`       | Main already persisted dispatch intent and did not blindly replay `INTENT_PERSISTED`/`CALL_RETURNED`, but an expired `NOT_STARTED` execution could still dispatch late and a matching observation could win after the deadline. Deadline-first recovery and persisted confirmation state are implemented in the working tree and await final verification. |
...
This is a current-baseline audit, not the final test report. Focused and full-repository outcomes,
the final merged-main SHA, and the candidate SHA belong in `test-results.json` and
`final-report.md` after the final clean-tree rerun.
