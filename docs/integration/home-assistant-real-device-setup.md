...
Real writes are not enabled by default. A controlled run must provide both `ALLOW_REAL_DEVICE_SIDE_EFFECTS=YES` and a unique `REAL_DEVICE_TEST_RUN_ID`; the driver enforces a per-run write budget and records the original state before any write.
