...
const shots = [
  ...
  ["runtime-deployment-create", "/runtime/deployments/new"], ["runtime-deployment-detail", "/runtime/deployments/deploy-001"],
  ["runtime-process-detail", "/runtime/processes/runtime-001"], ["configuration-draft", "/configuration/draft-001"],
  ...
  ["resource-detail", "/resources/ugv-01"], ["catalog-operation", "/catalog/providers/ugv-prod-001/io.sdar%2Fnavigation%2FnavigateTo"],
...
test("capture required product review screenshots", async ({ page }) => {
  for (const [name, path] of shots) {
    ...
    await page.screenshot({ path: `../../docs/design/pms-web-complete/screenshots/${name}.png`, fullPage: true });
  }
});
