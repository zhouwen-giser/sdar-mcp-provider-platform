...
describe("runtime deployment contract behavior", () => {
  ...
  it("accepts only the current desired revision", async () => {
    ...
    await expect(
      gateways.runtime.restartDeployment(
        deployment.deploymentId,
        {
          providerId: deployment.providerId,
          expectedDesiredRevision: deployment.desiredRevision - 1,
        },
        context,
      ),
    ).rejects.toMatchObject({
      problem: { code: "RUNTIME_DEPLOYMENT_REVISION_CONFLICT", status: 409 },
    });
    const intent = await gateways.runtime.stopDeployment(
      deployment.deploymentId,
      { providerId: deployment.providerId, expectedDesiredRevision: deployment.desiredRevision },
      context,
    );
    expect(intent.deployment.desiredState).toBe("stopped");
...
describe("configuration contract behavior", () => {
  it("validates, publishes and rolls back using explicit concurrency fields", async () => {
    ...
    const publication = await gateways.configuration.publishDraft(
      draft.draftId,
      { expectedDraftVersion: validated.version, expectedPublishedRevision: 3 },
      context,
    );
    expect(publication.outcome).toBe("published");
    const rollback = await gateways.configuration.rollbackDraft(
      draft.draftId,
      {
        expectedDraftVersion: validated.version,
        expectedPublishedRevision: publication.revision.revision,
        sourceRevisionId: "223e4567-e89b-42d3-a456-426614174000",
      },
      context,
    );
  ...
  it("supports deterministic no-change publication", async () => {
    ...
    const result = await gateways.configuration.publishDraft(
      draft.draftId,
      { expectedDraftVersion: draft.version, expectedPublishedRevision: 3 },
      context,
    );
