...
describe("provider and resource contract behavior", () => {
  it("creates a draft provider without onboarding side effects", async () => {
    ...
    expect(
      (await gateways.runtime.listDeployments(created.providerId, context)).items.some(
        (item) => item.providerId === created.providerId,
      ),
    ).toBe(false);
  });
  ...
  it("enforces expectedUpdatedAt for provider status", async () => {
    ...
    await expect(
      gateways.providers.updateProviderStatus(
        provider.providerId,
        { status: "degraded", expectedUpdatedAt: "stale" },
        context,
      ),
    ).rejects.toMatchObject({ problem: { code: "OPTIMISTIC_CONCURRENCY_CONFLICT", status: 409 } });
    const updated = await gateways.providers.updateProviderStatus(
      provider.providerId,
      { status: "degraded", expectedUpdatedAt: provider.updatedAt ?? "" },
      context,
    );
  ...
  it("binds and unbinds resources with duplicate protection", async () => {
    ...
    expect((await gateways.resources.listBindings("npc-training-001", context)).items).toHaveLength(
      1,
    );
    await expect(gateways.resources.bind("npc-training-001", input, context)).rejects.toMatchObject(
      { problem: { code: "DUPLICATE_RESOURCE_BINDING" } },
    );
    await gateways.resources.unbind(
      "npc-training-001",
      input.environment,
      input.resourceId,
      context,
