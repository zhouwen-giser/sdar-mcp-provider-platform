...
export class ContractMockStore implements ProviderGateway, ResourceGateway, ConfigurationGateway, RuntimeGateway, RegistryGateway, AuditGateway, ScenarioController {
  ...
  async listDeployments(context?: GatewayContext) { await this.gate(context); let data = clone(this.deploymentData); if (this.scenario === "empty") data = []; if (this.scenario === "runtime-failed" && data[0]) data[0] = { ...data[0], status: "FAILED", observedRevision: Math.max(0, data[0].desiredRevision - 1) }; if (this.scenario === "partial-data") data = data.slice(0, 1); return page(data); }
  async getDeployment(id: string, context?: GatewayContext) { await this.gate(context); const found = (await this.listDeployments()).items.find(x => x.deploymentId === id); if (!found) this.raise("RUNTIME_DEPLOYMENT_NOT_FOUND", 404, "Runtime deployment not found", context); return clone(found); }
  async createDeployment(input: RequestBody<"createRuntimeDeployment">, context?: GatewayContext) { await this.gate(context); if (this.deploymentData.some(x => x.deploymentId === input.deploymentId)) this.raise("ENTITY_ALREADY_EXISTS", 409, "Deployment already exists", context); const deployment: RuntimeDeploymentDto = { deploymentId: input.deploymentId, providerId: input.providerId, environment: input.environment, desiredState: input.desiredReplicas === 0 ? "stopped" : "running", desiredReplicas: input.desiredReplicas ?? 1, runtimeVersion: input.runtimeVersion, databaseProfileId: input.databaseProfileId, configProfileId: input.configProfileId, adapterEndpoint: input.adapterEndpoint, status: "REQUESTED", desiredRevision: 1, observedRevision: 0 }; this.deploymentData.push(deployment); const intent = { operationId: context?.correlationId ?? `corr-${deployment.deploymentId}-create`, deployment }; this.record("runtime_deployment.created", "runtime_deployment", deployment.deploymentId, context); return clone(intent); }
  ...
  async listProcesses(context?: GatewayContext) { await this.gate(context); let data = clone(this.processData); if (this.scenario === "empty") data = []; if (this.scenario === "runtime-failed" && data[0]) data[0] = { ...data[0], processState: "errored", readinessState: "not_ready", registrationState: "stale", catalogState: "stale", observedHealth: "FAILED", readyForActive: false, stale: true, registrationFreshness: "stale" }; if (this.scenario === "partial-data") data = data.slice(0, 1); return page(data); }
  async getProcess(id: string, context?: GatewayContext) { await this.gate(context); const found = (await this.listProcesses()).items.find(x => x.instanceId === id); if (!found) this.raise("RUNTIME_PROCESS_NOT_FOUND", 404, "Runtime process not found", context); return clone(found); }
  ...
  async list(context?: GatewayContext) { await this.gate(context); return page(this.scenario === "empty" ? [] : this.auditData); }
}
