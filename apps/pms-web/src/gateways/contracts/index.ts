...
export interface ProviderGateway {
  ...
  getProviderPackage(packageId: string, version?: string, context?: GatewayContext): Promise<ProviderPackageDto>;
  ...
  createProvider(input: RequestBody<"createProvider">, context?: GatewayContext): Promise<ProviderDto>;
...
export interface ResourceGateway {
  ...
  listBindings(providerId: string, context?: GatewayContext): Promise<Page<ProviderResourceBindingDto>>;
  bind(providerId: string, input: RequestBody<"bindProviderResource">, context?: GatewayContext): Promise<ProviderResourceBindingDto>;
  unbind(providerId: string, environment: string, resourceId: string, context?: GatewayContext): Promise<void>;
}
export interface ConfigurationGateway {
  createDraft(input: RequestBody<"createConfigurationDraft">, context?: GatewayContext): Promise<ConfigurationDraftDto>;
  getDraft(draftId: string, context?: GatewayContext): Promise<ConfigurationDraftDto>;
  updateDraft(draftId: string, input: RequestBody<"updateConfigurationDraft">, context?: GatewayContext): Promise<ConfigurationDraftDto>;
  validateDraft(draftId: string, context?: GatewayContext): Promise<ConfigurationDraftDto>;
  previewDraft(draftId: string, context?: GatewayContext): Promise<EffectiveConfigurationPreviewDto>;
  publishDraft(draftId: string, input: RequestBody<"publishConfigurationDraft">, context?: GatewayContext): Promise<ConfigurationPublicationResultDto>;
...
export class GatewayProblem extends Error {
  constructor(readonly problem: ProblemDetailsDto) { super(problem.code); }
}
