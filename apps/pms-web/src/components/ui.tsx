...
export function StatusBadge({ status }: { readonly status: string }) {
  return (
    <span className={`status status-${status.toLowerCase().replaceAll("_", "-")}`}>● {status}</span>
  );
}
...
export function PageHeader({
  ...
  readonly classification?:
    "FROZEN_API" | "WEB_COMPOSED" | "CLIENT_ONLY" | "DEFERRED" | "FORBIDDEN";
}) {
  return (
    <header className="page-header">
      <div>
        ...
        <div className="heading-line">
          <h1>{title}</h1>
          {classification === undefined ? null : <ClassificationBadge value={classification} />}
        </div>
        <p>{description}</p>
      </div>
...
export function ClassificationBadge({ value }: { readonly value: string }) {
  return (
    <span className={`classification classification-${value.toLowerCase().replaceAll("_", "-")}`}>
      {value}
    </span>
  );
}
        ...
        {reasonRequired ? (
          <label className="form-field">
            ...
            <textarea
              ...
              placeholder="至少 4 个字符；仅作为当前交互记录"
            />
          </label>
        ) : null}
        {requirePhrase === undefined ? null : (
          <label className="form-field">
            <span>
              输入 <code>{requirePhrase}</code> 完成二次确认
            </span>
  ...
  return (
    <ol className="timeline">
      {items.map((item, index) => (
        <li key={`${item.label}-${item.meta ?? index}`}>
  ...
  return (
    <section className="state-panel state-error">
      ...
      {problem.correlationId === undefined ? null : (
        <small>
          Correlation ID: <code>{problem.correlationId}</code>
        </small>
      )}
      {action}
    </section>
...
export function Toast({
  children,
  tone = "success",
}: PropsWithChildren<{ readonly tone?: "success" | "error" | "info" }>) {
  return (
...
export function DeferredCapability({
  title,
  reason,
  children,
}: PropsWithChildren<{ readonly title: string; readonly reason: string }>) {
  return (
    <section className="panel deferred-panel">
      <div>
        <ClassificationBadge value="DEFERRED" />
        <h2>{title}</h2>
        <p>{reason}</p>
      </div>
      {children}
...
export function LocalOnlyNotice({ children }: PropsWithChildren) {
  return (
    <section className="local-only-notice">
      <ClassificationBadge value="CLIENT_ONLY" />
      <span>{children}</span>
    </section>
  );
}
export function QuerySurface<T>({
  query,
  children,
  emptyTitle = "暂无数据",
}: {
  ...
  if (query.data === undefined)
    return <EmptyState title={emptyTitle} description="查询未返回可展示的数据。" />;
  return (
    <>
      {query.isFetching ? <div className="refresh-indicator">正在后台刷新</div> : null}
      {query.isStale ? <div className="stale-indicator">缓存数据可能已过期</div> : null}
      {children(query.data)}
    </>
  );
}
export function KeyValueList({ entries }: { readonly entries: readonly [string, ReactNode][] }) {
  return (
...
export function Tabs({
  ...
  return (
    <nav className="tabs" aria-label="页面区域">
      {items.map((item) => (
        <button
          key={item.id}
...
export function SimpleForm({
  onSubmit,
  children,
}: PropsWithChildren<{ readonly onSubmit: () => void }>) {
  return (
    <form
