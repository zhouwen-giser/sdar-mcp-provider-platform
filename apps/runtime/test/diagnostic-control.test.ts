...
describe("SMPP Benchmark diagnostic control API", () => {
  it.each([false, true])(
    "proxies the frozen contract with credentialFree=%s",
    async (credentialFree) => {
      ...
      const runtime = createRuntime(
        loadRuntimeConfig({
          ...
          SIMULATOR_CREDENTIAL_FREE: String(credentialFree),
      ...
      expect(unauthorized.statusCode).toBe(credentialFree ? 200 : 401);
